...
enum STATUS_ORDER { 'PAID', 'UNPAID', 'SHIPPING' }
...
const createPayment = async (req: Request, res: Response) => {
    ...
    let locale = 'vn'
    let currCode = 'VND'
    let material_fee = await Promise.all([getOrder(orderId), getValueOfCoupon(couponCode)])
    let amount = totalPayMent(material_fee[0], material_fee[1])
